class Scrape
  ...
  attr_accessor :title, :hotness, :image_url,
                :rating, :director, :genre, :release_date, :runtime,
                :synopsis, :failure
  ...
  def scrape_new_movie(url)
    begin
      ...
      # if ! self.synopsis.valid_encoding?
      #   self.synopsis = self.synopsis.encode("UTF-16be", :invalid=>:replace, :replace=>"?").encode('UTF-8')
      # end
